...
def stage_1_classify(page_text):
    """Classify the page text using Llama model - GENERIC VERSION"""
    prompt = f"""
You are an expert Financial Analyst analyzing a page from an annual report.

YOUR TASK: Determine if this page contains information about CONTINGENT LIABILITIES.

WHAT ARE CONTINGENT LIABILITIES?
- Uncertain future obligations that depend on specific events
- Legal disputes, lawsuits, court cases, litigation
- Tax disputes, claims not acknowledged as debt
- Potential liabilities that may or may not materialize
- Different from firm commitments or guarantees already given

WHAT TO LOOK FOR:
✓ Any table/section with "Contingent Liabilit" in the title (any variation)
✓ Legal proceedings, litigation tables
✓ Claims/disputes not acknowledged as debt
✓ Combined tables like "Contingent Liabilities and Commitments" (ACCEPT - contains contingent liabilities)
✓ Tax demands/disputes under appeal

WHAT TO REJECT:
✗ ONLY "Commitments" without any contingent liabilities
✗ ONLY "Guarantees" (bank guarantees, performance guarantees) without contingent liabilities
✗ Capital commitments alone
✗ Purchase obligations alone

DECISION RULE:
If the page mentions contingent liabilities IN ANY FORM (standalone OR combined with other items), mark as Relevant.
Only reject if it's purely about commitments/guarantees with NO mention of contingent liabilities.
...
def stage_2_classify(page_text):
    """Verify the page classification - GENERIC VERSION"""
    prompt = f"""
You are performing a VERIFICATION check on a page that was flagged as containing contingent liabilities.

VERIFY: Does this page actually contain CONTINGENT LIABILITIES information?

CONTINGENT LIABILITIES are:
- Uncertain future obligations (legal cases, tax disputes, claims)
- Potential liabilities depending on uncertain future events
- Items "not acknowledged as debt"
- Different from firm commitments or standard guarantees

ACCEPT IF:
✓ Title/heading contains "Contingent Liabilit" (any spelling/variation)
✓ Discusses legal proceedings, litigation, lawsuits
✓ Shows tax disputes under appeal
✓ Combined sections like "Contingent Liabilities and Commitments"
✓ Has tabular data about uncertain obligations

REJECT IF:
✗ ONLY about commitments (no uncertain liabilities)
✗ ONLY about guarantees (no uncertain liabilities)
✗ Just mentions the term but no actual table/data
✗ Only capital expenditure commitments

FINAL CHECK:
Does this page have a TABLE or DETAILED INFORMATION about contingent liabilities?
If YES → Relevant
If NO → Non Relevant
...
def classifyTable_with_context_check(table_markdown: str = "", table_index: int = 0, full_document_markdown: str = ""):
    """Enhanced classification - GENERIC VERSION with context awareness"""
    prompt = f"""
You are a financial document analyzer with expertise in identifying contingent liabilities.

FULL DOCUMENT CONTEXT (first 3000 chars):
{full_document_markdown[:3000]}

SPECIFIC TABLE TO ANALYZE:
{table_markdown}

YOUR TASK: Determine if this table contains CONTINGENT LIABILITIES data.

WHAT ARE CONTINGENT LIABILITIES?
Uncertain future obligations that may or may not occur, such as:
- Legal disputes, lawsuits, court cases, arbitration
- Tax demands under appeal or contested
- Claims/demands not acknowledged by the company
- Potential obligations depending on uncertain events
- Litigation outcomes pending

HOW TO DECIDE:

1. CHECK SECTION HEADING (in document context above):
   - If section is titled "Contingent Liabilit..." → LIKELY TRUE
   - If section is "Legal Proceedings" or "Litigation" → LIKELY TRUE
   - If section is "Contingent Liabilities and Commitments" → TRUE (contains contingent liabilities)
   - If section is ONLY "Commitments" or ONLY "Guarantees" → Check table content

2. CHECK TABLE CONTENT:
   - Does it list legal cases, court matters, disputes? → TRUE
   - Does it show tax demands under appeal? → TRUE
   - Does it mention "not acknowledged as debt"? → TRUE
   - Does it describe uncertain future payments? → TRUE
   - Is it about purchase orders, capital commitments only? → FALSE
   - Is it about bank guarantees given (without disputes)? → FALSE

3. COMBINED TABLES:
   - Tables with BOTH contingent liabilities AND commitments → TRUE (we want the contingent liabilities part)
   - Tables with ONLY commitments/guarantees (no uncertain liabilities) → FALSE

FINAL DECISION LOGIC:
- If table contains ANY information about uncertain liabilities (lawsuits, tax disputes, claims) → True
- If table is ONLY about firm commitments or standard guarantees → False
- When in doubt and table mentions "contingent" anywhere → True
- If completely unrelated to liabilities → False

OUTPUT FORMAT:
Respond with ONLY one word: "True" or "False"
- True = This table contains contingent liabilities information
- False = This table does NOT contain contingent liabilities information
...
def fix_merged_columns(df):
    ...
    if fixes_made > 0:
        ...
        print(f"    ✓ No column merging issues detected")
...
def create_table_with_full_features(OUTPUT_PDF_PATH):
    ...
    2. Column merge fixing
    3. Currency and unit extraction
